export const vehicleFeatures = [
  {
    title: "Vehicle compatibility",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "VIN scanner",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Vehicle profiles",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Real- time vehicle telematics ",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Ai based vehicle body inspection",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Dynamic body condition visualization",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Hardware-free GPS fleet tracking",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Vehicle document management",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Remote digital key",
    permissions: [{ permission: false }, { permission: true }],
  },
];

export const fleetFeatures = [
  {
    title: "Automated fleet health report",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Advanced fleet analytics",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Dynamic expenses management",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "AI driven maintenance management",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Predictive maintenance",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Maintenance alerts",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Documents expiration alerts",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Mileage reminders",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Scheduled reminders",
    permissions: [{ permission: false }, { permission: true }],
  },
  {
    title: "Documents expiration alerts",
    permissions: [{ permission: false }, { permission: true }],
  },
];

export const brandFeatures = [
  {
    title: "Multi-location fleet management",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "BEE - Ai chat bot",
    permissions: [
      { permission: true, message: "Fleet assistant" },
      { permission: true, message: "Garage manager" },
    ],
  },
];

export const teamFeatures = [
  {
    title: "Fleet team management",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Assigned fleet managers",
    permissions: [{ permission: true }, { permission: true }],
  },
  {
    title: "Smart employee app",
    permissions: [{ permission: true }, { permission: true }],
  },
];
